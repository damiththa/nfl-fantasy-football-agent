...
def get_weekly_matchup(
    league: Any, team_id: int, week: int, league_config: LeagueConfig
    ...
    for matchup in box_scores:
        ...
        if home_team_id == team_id or away_team_id == team_id:
            ...
            your_projected = (
                getattr(matchup, "home_projected", 0.0)
                if is_home
                else getattr(matchup, "away_projected", 0.0)
            )
            opp_projected = (
                getattr(matchup, "away_projected", 0.0)
                if is_home
